...
PR (pull request) создаётся с помощью ссылки, которую GitHub выводит через Git в консоль после выполнения команды `git push`. GitHub вливает изменения в ветку `main` в `origin`, а локальная ветка остаётся без изменений. После "мёржа" PR рекомендуем обновить локальную `main`: `git checkout main && git pull`.
