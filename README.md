...
* `\` - объединяет строки, в конце консольной команды позволяет продлить её на следующую строку;
...
* Просмотреть изменения "глазами" — это называется __code review__.

PR (pull request) создаётся с помощью ссылки, которую GitHub выводит через Git в консоль после выполнения команды `git push`. GitHub вливает изменения в ветку `main` в `origin`, а локальная ветка остаётся без изменений. После "мёржа" PR рекомендуется обновить локальную `main`: `git checkout main && git pull`.
...
* Создать ветку для вашей задачи: `git checkout -b <название_ветки>`.
* Добавить и закоммитить изменения, которые вы хотите внести в проект.
* Запушить ветку: `git push --set-upstream origin HEAD` или `git push -u origin <название_ветки>`.
...
* Обсуждать с ревьюером предлагаемые изменения и вносить правки, пока эти изменения не будут одобрены (пока не будет получен «апрув»).  
  * Если кто-то добавил конфликтующие изменения в `main`, пока ваш PR был на ревью, нужно разрешить конфликт:  
    ...
    * Влить `main` в свою ветку: `git checkout <название_ветки> && git merge main`.
...
__Конфликты слияния__ могут случаться при слиянии веток (merge), если один и тот же файл был изменён в обеих ветках. Git не может сам решить, какая версия файла «правильная», а какая нет. Вывод будет таким: 

`Auto-merging readme.md
CONFLICT (content): Merge conflict in readme.md
Automatic merge failed; fix conflicts and then commit the result.`

При попытке объединить ветки или применить изменения из удалённого репозитория Git добавит в файлы специальные маркеры конфликта. Чтобы разрешить конфликт вручную, нужно открыть файл и выбрать, какие изменения оставить, а какие отбросить. Для этого следует удалить все маркеры и ненужные изменения и оставить нужные. После разрешения конфликта файлы будут отмечены как решённые. Можно продолжить процесс слияния или выполнить коммит изменений.

`git mergetool` - вызвать инструмент слияния, например `vimdiff`.  

### Разрешение конфликтов с помощью `vimdiff`
`vimdiff` показывает четыре окна:  
* В верхнем левом углу — текущая версия файла в `HEAD`;
* В правом верхнем углу — версия из новой ветки;
...
`vimdiff` создаёт копию конфликтного файла с маркерами изменений и расширением .orig. Этот файл можно удалить после слияния. После успешного слияния выполняем коммит. Он станет коммитом слияния. Такую же процедуру можно произвести с помощью VS Code. 
